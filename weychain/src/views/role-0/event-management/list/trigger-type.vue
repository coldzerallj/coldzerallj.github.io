			...
			<el-row>
				...
				<el-col :md="4" :lg="4">
					<el-form-item label="规则状态">
						<el-select v-model="filter.status" style="width:100%;" placeholder="请选择规则状态">
...
export default {
	...
	methods: {
		...
		async setRequestUrl() {
			...
			this.table.url = api.eventManagement.getOprTriggerActList(data);
		},
		// 接口请求完筛选数据
		filterData() {
			// 规则状态,1-已上线 4-已下线
			...
			this.$refs['sectionTable'].table.tbody.map((item, index) => {
				item.zhStatus = status[item.status];
		...
		async tbtnClick(index, row, prop) {
			if (prop === '查看生效记录') {
				// 存储
				sessionStorage.setItem('triggerId', row.id);
				this.$router.push({
					name: this.$ALL_CONST.ROUTE_NAME.RN_TRIGGER_TYPE_RECORD,
					query: {
						from: this.$ALL_CONST.ROUTE_NAME.RN_TRIGGER_TYPE,
					},
				});
				...
				const postData = {
					id: row.id,
					status: row.status === 4 ? 1 : 4,
				};
				const res = await api.eventManagement.oprTriggerActGrounding(postData);
				if (res.code === 200) {
					this.setRequestUrl();
